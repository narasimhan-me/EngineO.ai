...
describe('TEST-3 – Live Shopify Smoke Runner (dry mode)', () => {
  ...
  function runSmokeScript(
    ...
    const fullEnv = { ...setValidEnv(), ...env };
    const envString = Object.entries(fullEnv)
      .map(([k, v]) => `${k}="${v}"`)
      .join(' ');

    const scriptPath = 'apps/api/scripts/shopify-live-smoke.ts';
    const command = `${envString} npx ts-node ${scriptPath} ${args.join(' ')}`;
  ...
  describe('dry run mode', () => {
    it('validates configuration without making Shopify calls', () => {
      ...
      expect(result.stdout).toContain('Configuration validated successfully');
      expect(result.stdout).toContain('test-store.myshopify.com');
    });
